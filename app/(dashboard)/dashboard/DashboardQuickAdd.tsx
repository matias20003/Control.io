...
type SaveMode = "save" | "save_and_another";
...
/* ── Formulario ──────────────────────────────────────────────────────────────
   Vive en el scope del módulo (no dentro del render del padre): si se declara
   adentro, cada render del padre crea un tipo de componente nuevo y React
   desmonta/remonta todo el form, borrando lo tipeado en pleno guardado.       */
function QuickForm({
  accounts,
  categories,
  txType,
  ...
  isPending: boolean;
  onCreate: (formData: FormData, mode: SaveMode) => void;
  onCancel: () => void;
}) {
  ...
  // "Guardar" es el único submit real del form, así que Enter también guarda y
  // cierra. "+ Otro" pide el submit a mano marcando el modo en este ref.
    ...
    <>
      <div className="flex rounded-xl overflow-hidden border border-border mb-5">
        {(["EXPENSE", "INCOME", "TRANSFER"] as TxType[]).map((t) => (
          <button key={t} type="button" onClick={() => onTxTypeChange(t)}
      ...
      <form ref={formRef} action={handleAction} className="space-y-4">
        ...
        <div className="mobile-form-actions desktop-form-full flex gap-2 pt-1 lg:justify-end">
          <Button type="button" variant="ghost" className="flex-shrink-0 px-3" onClick={onCancel}>
            Cancelar
          </Button>
          <Button type="button" variant="outline" className="flex-1" disabled={isPending}
            onClick={() => { modeRef.current = "save_and_another"; formRef.current?.requestSubmit(); }}>
            + Otro
          </Button>
...
export function DashboardQuickAdd({ accounts, categories }: Props) {
  ...
  const closeModal = () => { setIsOpen(false); setLastDefaults({}); };
  ...
  const handleCreate = (formData: FormData, mode: SaveMode) => {
    if (txType === "TRANSFER") {
    ...
    <>
      ...
      <Dialog open={isOpen} onOpenChange={(o) => { if (!o) closeModal(); }}>
        <DialogContent title="Nuevo movimiento">
          {/* La key incluye el tipo: cambiar de pestaña reinicia el form (y la
              categoría elegida), igual que antes, sin remontarlo en cada render. */}
          <QuickForm
            key={`${formKey}-${txType}`}
            accounts={accounts}
