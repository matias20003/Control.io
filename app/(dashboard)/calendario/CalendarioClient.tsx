...
import { ChevronLeft, ChevronRight, Plus, Trash2, Pencil, Loader2, CalendarDays } from "lucide-react";
...
type Modal = { mode: "create" | "edit"; id?: string; title: string; day: string; time: string };
...
export function CalendarioClient({
  ...
  const openCreate = (day: string) => setModal({ mode: "create", title: "", day, time: "09:00" });
  const openEdit = (it: CalItem, day: string) => {
    if (it.kind !== "event" || !it.id) return;
    setModal({ mode: "edit", id: it.id, title: it.label, day, time: it.time || "09:00" });
  };
  ...
  const save = async () => {
    ...
    const res = modal.mode === "create"
      ? await createCalendarEventAction({ title: modal.title, day: modal.day, time: modal.time })
      : await updateCalendarEventAction({ id: modal.id!, title: modal.title, day: modal.day, time: modal.time });
    setSaving(false);
    if ("error" in res) { toast.error(res.error); return; }
    toast.success(modal.mode === "create" ? "Evento creado ✓" : "Evento actualizado ✓");
    setModal(null);
    router.refresh();
  };
  ...
  const del = async () => {
    if (!modal?.id) return;
    setSaving(true);
    const res = await deleteCalendarEventAction(modal.id);
    setSaving(false);
    if ("error" in res) { toast.error(res.error); return; }
  ...
  return (
    <div className="p-3 md:p-6 max-w-[1100px] mx-auto space-y-3">
      ...
      {selectedCell && (
        <div className="rounded-2xl border border-border bg-surface p-4">
          ...
          {selectedCell.items.length === 0 ? (
            <p className="py-2 text-xs text-muted">Nada agendado este día.{googleConnected ? " Tocá “Evento” para agregar." : ""}</p>
          ) : (
            <div className="divide-y divide-border-subtle">
              {selectedCell.items.map((it, i) => (
                ...
                >
                  <span className={`h-2 w-2 shrink-0 rounded-full ${DOT[it.kind]}`} />
                  <span className="min-w-0 flex-1 truncate text-sm text-foreground">
                    {it.time && <span className="text-muted">{it.time} · </span>}{it.label}
                  </span>
                  {it.kind === "event" && <Pencil size={13} className="shrink-0 text-muted" />}
                </button>
              ))}
            </div>
          )}
      ...
      <Dialog open={!!modal} onOpenChange={(o) => { if (!o) setModal(null); }}>
        <DialogContent title={modal?.mode === "edit" ? "Editar evento" : "Nuevo evento"}>
          {modal && (
            <div className="space-y-3">
              <Input value={modal.title} onChange={(e) => setModal({ ...modal, title: e.target.value })} placeholder="¿Qué es? (ej: Turno médico)" autoFocus />
              <div className="flex gap-2">
                <Input type="date" value={modal.day} onChange={(e) => setModal({ ...modal, day: e.target.value })} className="flex-1" />
                <Input type="time" value={modal.time} onChange={(e) => setModal({ ...modal, time: e.target.value })} className="w-28" />
              </div>
              <div className="flex gap-2 pt-1">
                {modal.mode === "edit" && (
                  <Button variant="danger" onClick={del} disabled={saving} aria-label="Borrar"><Trash2 size={15} /></Button>
                )}
                <Button variant="ghost" className="flex-1" onClick={() => setModal(null)} disabled={saving}>Cancelar</Button>
                <Button className="flex-1" onClick={save} disabled={saving || !modal.title.trim()}>
                  {saving ? <Loader2 size={15} className="animate-spin" /> : <><CalendarDays size={15} className="mr-1" /> Guardar</>}
                </Button>
              </div>
            </div>
          )}
